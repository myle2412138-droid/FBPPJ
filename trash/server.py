...
def send_data_to_api(report_data):
    """Gửi dữ liệu báo cáo lên API PHP"""
    api_url = "https://viegrand.site/phpfpb/api.php"
    try:
        print(f"🚀 Đang gửi dữ liệu lên {api_url}...")
...
def upload_file_to_php(file_path):
    """Upload file lên PHP server và trả về URL"""
    upload_url = "https://viegrand.site/phpfpb/upload.php"
    try:
...
def create_video():
    ...
    try:
                ...
                try:
                    ...
                    if has_tumor and len(detected_frames) < 5:
                        try:
                            ...
                            print(f"⚠️ Error saving tumor image: {e_img}")
        ...
        remote_video_url = upload_file_to_php(output_video_path)
        
        remote_detected_frames = []
        for local_img_path in detected_frames:
            remote_url = upload_file_to_php(local_img_path)
            if remote_url:
                remote_detected_frames.append(remote_url)
            else:
                # Fallback to local relative path if upload fails
                remote_detected_frames.append(f'/results/{os.path.basename(local_img_path)}')

        # Use remote URL if available, else local
        final_video_url = remote_video_url if remote_video_url else f'/results/{output_video_name}'
        ...
        try:
            ...
            report_lines.append('Tài liệu kèm theo:')
            report_lines.append(f'- Video kết quả: {final_video_url}')
            for p in remote_detected_frames:
            ...
            report = {
                ...
                'detected_frames': remote_detected_frames,
...
def analyze_video():
    ...
    try:
        ...
        # Upload video
        remote_video_url = upload_file_to_php(video_path)
        final_video_url = remote_video_url if remote_video_url else f'/results/{video_name}'
